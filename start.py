...
TAX_PL = 0.88  # Uwzględnia 12% podatku (1.0 - 0.12)
MIN_EDGE = 0.02 
...
def fetch_odds(league_key):
    for key in API_KEYS:
        ...
        if r.status_code == 429: print(f"⚠️ Klucz wyczerpany, rotacja...")
    return None

def run_scanner():
    print(f"🔍 SKAN Z PODATKIEM 12%: {datetime.now().strftime('%H:%M:%S')}")
    ...
    for l_key, l_name in LEAGUES.items():
        ...
        for e in events:
            ...
            for sel, prob in probs.items():
                o = best_odds[sel]
                # EDGE obliczany z uwzględnieniem podatku (Kurs * 0.88)
                edge = (prob - 1/(o * TAX_PL))
                
                if edge >= MIN_EDGE and f"{home}_{sel}" not in existing_ids:
                    stake = 100.0
                    msg = (f"🎯 <b>NOWY TYP ({l_name})</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                           f"🏟 <b>{home} - {away}</b>\n📅 {dt.astimezone().strftime('%d.%m, %H:%M')}\n\n"
                           f"🔸 Typ: <b>{sel}</b>\n🔹 Kurs: <b>{o}</b> (netto: {round(o*0.88, 2)})\n\n"
                           f"📈 Edge (z pod.): <b>+{edge*100:.1f}%</b>\n💰 Stawka: <b>{stake}j</b>\n"
                           f"━━━━━━━━━━━━━━━━━━━━\nStatus: PENDING")
                    send_msg(msg)
                    coupons.append({"home": home, "away": away, "pick": sel, "odds": o, "stake": stake, "status": "PENDING", "league_key": l_key, "league_name": l_name, "date": dt.isoformat(), "edge": round(edge*100, 2)})

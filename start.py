...
def run():
    ...
    for sport_key, sport_label in SPORTS_CONFIG.items():
        ...
        for m in matches:
            ...
            common_data = {
                "id": m["id"], "league": sport_label, "key": sport_key, "date": m_dt_utc,
    ...
    for s in singles:
        ...
        msg = (
            f"🎯 *TYP SINGLE*\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🏟️ **Mecz:** `{s['home_name']} vs {s['away_name']}`\n"
            f"✅ **Typ:** `{s['picked']}`\n"
            f"🏆 **Liga:** {s['league']}\n"
            f"📅 **Start:** `{match_time}`\n"
            f"📈 **Kurs:** `{s['odd']:.2f}`\n"
    ...
    while len(all_picks) >= 2:
        ...
        t1 = (p1["date"] + timedelta(hours=1)).strftime('%d.%m %H:%M')
        t2 = (p2["date"] + timedelta(hours=1)).strftime('%d.%m %H:%M')
        ...
        msg = (
            ...
            f"   ✅ Typ: `{p1['picked']}` | `{p1['odd']:.2f}`\n"
            f"   📅 Start: `{t1}`\n\n"
            f"2️⃣ `{p2['home_name']} vs {p2['away_name']}`\n"
            f"   ✅ Typ: `{p2['picked']}` | `{p2['odd']:.2f}`\n"
            f"   📅 Start: `{t2}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 **AKO:** `{ako:.2f}`\n"
            f"💰 **Stawka:** `{STAKE_STANDARD} PLN`\n"
            f"💰 **Wygrana:** `{win} PLN`"
        )
        send_msg(msg)
        coupons_db.append({

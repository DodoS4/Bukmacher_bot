...
def format_value_card(league_key, home, away, dt, pick, odds, edge, stake, type_desc="VALUE"):
    info = LEAGUE_INFO.get(league_key, {"name": league_key, "flag": "🎯"})
    tier = "A" if edge >= 0.08 else "B"
    return (
        f"{info['flag']} <b>{type_desc} • {info['name']}</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>{home} vs {away}</b>\n"
...
def generate_over_btts_pick(match):
    # BTTS / Over 2.5
    # uproszczony przykład: szukamy kurs >=2.0 i edge >= 0.02
    over_odds = match.get("over_2_5") or 2.0
    btts_odds = match.get("btts") or 2.0
    picks = []
    if over_odds >= 2.0:
        picks.append({"sel":"Over 2.5","odds":over_odds,"val":0.02})
    if btts_odds >= 2.0:
        picks.append({"sel":"BTTS Yes","odds":btts_odds,"val":0.02})
    return picks
        ...
        for key in API_KEYS:
            try:
                ...
                for e in r.json():
                    ...
                    match_info = {"home":e["home_team"],"away":e["away_team"],"league":league,
                                  "odds":{"home":odds.get(e["home_team"]),"away":odds.get(e["away_team"]),"draw":odds.get("Draw")}}
                    
                    pick_value = generate_value_pick(match_info)
                    over_btts_picks = generate_over_btts_pick(match_info)

                    picks_to_send = []
                    if pick_value:
                        picks_to_send.append(("VALUE", pick_value))
                    for ob in over_btts_picks:
                        picks_to_send.append(("BTTS/OVER", ob))

                    for type_desc, pick in picks_to_send:
                        # Sprawdzenie duplikatów
                        if any(c for c in coupons if c["home"]==e["home_team"] and c["away"]==e["away_team"] and c["picked"]==pick["sel"] and c["sent_date"]==str(now.date())):
                            continue
                        ...
                        send_msg(format_value_card(league, e["home_team"], e["away_team"], dt, pick["sel"], pick["odds"], pick["val"], stake, type_desc=type_desc))
                break
            except: continue

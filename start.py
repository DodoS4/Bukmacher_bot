...
BANKROLL = 1000              
EV_THRESHOLD = 0.0           # Próg ustawiony na 0.0 zgodnie z Twoją prośbą
MIN_ODD = 1.40               
MAX_ODD = 4.50               
TAX_RATE = 0.88              
KELLY_FRACTION = 0.1         
...
def run():
    ...
    pol_date = datetime.now().strftime('%d.%m.%Y')
    
    # --- RAPORT STARTOWY (HEARTBEAT) ---
    active_keys = len(API_KEYS)
    start_msg = (f"🚀 **Bot wystartował!**\n"
                 f"📅 Data: `{pol_date}`\n"
                 f"⏰ Godzina (PL): `{pol_hour}:00`\n"
                 f"🔑 Aktywne klucze API: `{active_keys}`\n"
                 f"📊 Próg EV: `{EV_THRESHOLD}%`")
    send_msg(start_msg)
    ...
    # RAPORT KONCOWY (Zamiast ogólnego statusu, tylko gdy nie było ofert, by nie spamować)
    if opportunities_found == 0:
        summary_msg = (f"🏁 **Sesja zakończona**\n"
                       f"Przeanalizowano: `{total_scanned}` meczów\n"
                       f"Aktywne ligi: `{active_leagues}/{len(SPORTS_CONFIG)}`\n"
                       f"Najlepsze EV: `{max_ev_found:.2f}%` netto")

...
def scan_offers():
    ...
    for league in LEAGUES:
        ...
        for key in API_KEYS:
            try:
                ...
                total_selected += len(data)  # bez filtrów, test

                if len(data) > 0:
                    works.append((league, len(data)))
                found = True
                break
            except:
                continue
    ...
    # Debug testowy
    debug_msg = "🔍 Skanowanie ofert – BEZ FILTRÓW\n━━━━━━━━━━━━━━\n"
    for lg, cnt in works:
        debug_msg += f"✅ {lg}: {cnt} meczów\n"
    for lg in fails:
        debug_msg += f"❌ {lg} niedostępne\n"
    debug_msg += f"\nZeskanowano: {total_scanned} meczów\nWybrano: {total_selected} value-betów\n"
    debug_msg += f"\n✅ Działa: {len(works)} lig\n❌ Niedostępne: {len(fails)} lig\n"
    send_msg(debug_msg, "results")
